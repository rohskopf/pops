...
double fi[memchunk1];
double fi_ifc[memchunk1];
int ifc_counter;
vector<double> fi_ifc_vec;
int fio_indx;
...
double w_fc; // force constant weight
double w_fc3; // 3rd order force constant weight
int natoms;
double f[memchunk1];
double x[memchunk1];
double x_all[memchunk1]; //array of all positions from CONFIGS [x11, y11, z11, x21, y21, z21, ... xNM, yNM, zNM]
double x_all_ifc[memchunk1];//array of all IFC positions
int ifc_bool[memchunk1];
int M_indx;
double E0; // Reference energy for EV shape fitting
double E0_md;
double P0; // Reference energy for PV shape fitting
double P0_md;
double fc_ref_vec[memchunk1]; // Reference FCs for FC fitting
double fc3_ref_vec[memchunk1]; // Reference 3rd order FCs for FC fitting
int fc_num; // number of FCs
int fc3_num; // number of 3rd order FCs
...
void getinput()
{
  ...
  for (int i=1; i<=22; i++)
  {
    ...
    switch (i)
    {
      ...
      case 20:{ //cout << line << endl; // VECTOR INPUTS START HERE
                string temp;
               	vector<double> vec;
  ...
  for (int k=1; k<=M; k++)
  {
    ...
    getline(configs, line);
    //debug << line << endl;
    // Check tags in CONFIGS header
    std::size_t found = line.find(ifc_tag);
    if (found!=std::string::npos)
    {
      //debug << "We have an IFC config!" << endl;
      ifc_bool[k-1] = 1;
    ...
    { 
      ...
      if (ifc)
      {
        x_all_ifc[N_indx+coor_indx] = xcoor;
        x_all_ifc[N_indx+coor_indx+1]=ycoor;
        x_all_ifc[N_indx+coor_indx+2]=zcoor;
      }
    ...
    ss3 >> x_length >> y_length >> z_length;
  ...
  // Read the reference FCs
  ifstream fcref("FCREF");
  string blah1;
  string blah2;
  double fc;
  // Skip first line
  getline(fcref, line);
  // Loop through "fc_num" lines
  for (int i = 1; i <=fc_num; i++)
  {
    getline(fcref, line);
    stringstream ss(line);
    ss >> blah1 >> blah2 >> fc;
    // Add to reference fc vec
    fc_ref_vec[i-1] = fc;
  ...
  // Skip next line
  getline(fcref, line);
  // Loop through "fc3_num" lines
  for (int i = 1; i <=fc3_num; i++)
  {
    getline(fcref, line);
    stringstream ss(line);
    ss >> blah1 >> blah2 >> fc;
...
double objfun(double params[])
{
  ...
  int M_indx_ifc = 0;
  int t_indx = 0;
  double sum_fio_config = 0; // To check the sum of forces in a configuration
  double M_f = M; // To properly normalize the force configurations
  for (int i=0; i<M; i++)
  {  
      ...
      string box_size_str = "change_box all x final 0 LENGTH y final 0 LENGTH z final 0 LENGTH units box";
	...
	//double denom2 = pow( ( pow(fio_x, 2) + pow(fio_y, 2) + pow(fio_z, 2) ), 0.5  );
        ...
        sum_fio_config = sum_fio_config + pow(sqrt(pow(fio_x, 2) + pow(fio_y, 2) + pow(fio_z, 2) ), 2);
	...
	//debug << "denom2 " << denom2 << endl;*/
        ...
        if (ifc_bool[i] == 1) // If we have an IFC config
        {	
          /*debug << "fix: " << fi_x << endl;
          debug << "fiy: " << fi_y << endl;
          debug << "fiz: " << fi_z << endl;	
        ...
        if (denom != 0)
        {
          ...
          debug << "percent error: " << f_diff_mag/denom << endl;
        }
	...
	debug << "sum_f_diff: " << sum_f_diff << endl;*/
        ...
        if (ifc_bool[i] == 1) // If we have an IFC config
        {
          fi_ifc[forceindx + M_indx_ifc] = fi_x;
          fi_ifc[forceindx + 1 + M_indx_ifc] = fi_y;
          fi_ifc[forceindx + 2 + M_indx_ifc] = fi_z;
          //debug << fio_x << " " << fio_y << " " << fio_z << endl;
        }
      ...
      // Check if this configuration has sum_fio_config = 0
      //debug << "sum_fio_config: " << sum_fio_config << endl;
      if (sum_fio_config == 0)
      {
        //debug << "Volume config!" << endl;
        M_f = M_f - 1;
        //debug << "M_f: " << M_f << endl;
        debug << x_length << endl;
        debug << eio_s[i] << endl;
    ...
    } // for (int i=0; i<M; i++) END OF CONFIGS LOOP
  ...
  // CONFIGS loop is now complete...
  // Begin calculating IFCs
  // Need to extract positions and forces from IFC configs...
  // Forces are already stored in "fi_ifc", just need to write them to FFILE

  char ffile[64];
  sprintf (ffile, "FFILE%d", rank);
  ofstream ffile_fh(ffile);
  //int fi_ifc_size = fi_ifc.size();
  //ffile << fi_ifc_size << endl;
  int f_indx = 0;
  double fx, fy, fz;
  for (int i = 1; i <= N*ifc_counter; i++)
  {
	fx = fi_ifc[f_indx];
	fy = fi_ifc[1 + f_indx];
	fz = fi_ifc[2 + f_indx];
	// Convert forces from eV/ang to Ryd/Bohr
	fx = fx/25.71104309541616;
        fy = fy/25.71104309541616;
	fz = fz/25.71104309541616;
    //ffile_fh << fi_ifc[f_indx] << " " << fi_ifc[1 + f_indx] << " " << fi_ifc[2 + f_indx] << endl;
	ffile_fh << fx << " " << fy << " " << fz << endl;
    f_indx = f_indx+3;
  }
  ffile_fh.close();
  ...
  // Run alm
  char run_alm[64];
  sprintf (run_alm, "./alm alm%d.in > alm%d.log", rank, rank);
  //debug << run_alm << endl;
  //system("./alm alm.in > alm.log");
  system(run_alm);

  // Read the .fcs file
  //string line;
  char fcs[64];
  sprintf (fcs, "FCS%d.fcs", rank);
  ifstream fcs_fh(fcs);
  // Get to 16th line
  for (int i = 1; i <= 16; i++)
  {
  ...
  // Collect the next "fcs_num" FCs
  string blah1;
  string blah2;
  double fc;
  double sum_fc_diff = 0;
  double sum_fc_ref = 0;
  for (int i = 1; i <=fc_num; i++)
  {
    //debug << line << endl;

    // The third number is the FC
    stringstream ss(line);
    ss >> blah1 >> blah2 >> fc;
    //debug << "potential: " << fc << endl;
    //debug << "ref: " << fc_ref_vec[i-1] << endl;

    // Calculate error in FC
    double fc_diff = pow(fc_ref_vec[i-1] - fc,2);
    sum_fc_diff = sum_fc_diff + fc_diff;
    sum_fc_ref = sum_fc_ref + pow(fc_ref_vec[i-1], 2);

    //debug << "fc_diff: " << fc_diff << endl;
    //debug << "sum_fc_diff: " << sum_fc_diff << endl;
    //debug << "sum_fc_ref: " << sum_fc_diff << endl;
  ...
  getline(fcs_fh, line);
  getline(fcs_fh, line);
  // Collect the next "fcs3_num" FCs
  double fc3;
  double sum_fc3_diff = 0;
  double sum_fc3_ref = 0;
  for (int i = 1; i <=fc3_num; i++)
  {
    //debug << line << endl;

    // The third number is the FC3
    stringstream ss(line);
    ss >> blah1 >> blah2 >> fc3;
    debug << "potential: " << fc3 << endl;
    debug << "ref: " << fc3_ref_vec[i-1] << endl;

    // Calculate error in FC3
    double fc3_diff = pow(fc3_ref_vec[i-1] - fc3,2);
    sum_fc3_diff = sum_fc3_diff + fc3_diff;
    sum_fc3_ref = sum_fc3_ref + pow(fc3_ref_vec[i-1], 2);

    //debug << "fc3_diff: " << fc3_diff << endl;
    //debug << "sum_fc3_diff: " << sum_fc3_diff << endl;
    //debug << "sum_fc3_ref: " << sum_fc3_diff << endl;
  ...
  Z_2_f = (Z_2_f)/(M*N*3);
  Z_2_e = (Z_2_e)/(M);
  double Z_f = ((sum_f_diff)/(sum_fio*M_f*N));
  ...
  double Z_fc = ((sum_fc_diff)/(sum_fc_ref*fc_num));
  double Z_fc3 = ((sum_fc3_diff)/(sum_fc3_ref*fc3_num));
  double Z = w_f*Z_f + w_e*Z_e + w_s*Z_s + w_e_s*Z_e_s + w_p*Z_p + w_p_s*Z_p_s + w_fc*Z_fc + w_fc3*Z_fc3;

  debug << "------------------------------------------------------------------------" << endl;

  debug << "sum_f_diff: " << sum_f_diff << endl;
  debug << "sum_fio: " << sum_fio << endl;
  debug << "Z_f: " << Z_f << endl;
  debug << "w_f*Z_f: " << w_f*Z_f << endl;
  debug << "-------------" << endl;
  debug << "sum_e_diff: " << sum_e_diff << endl;
  debug << "sum_eio: " << sum_eio << endl;
  debug << "Z_e: " << Z_e << endl;
  debug << "w_e*Z_e: " << w_e*Z_e << endl;
  debug << "-------------" << endl;
  debug << "sum_e_diff_s: " << sum_e_diff_s << endl;
  debug << "sum_eio_s: " << sum_eio_s << endl;
  debug << "Z_e_s: " << Z_e_s << endl;
  debug << "w_e_s*Z_e_s: " << w_e_s*Z_e_s << endl;
  debug << "-------------" << endl;
  debug << "sum_s_diff: " << sum_s_diff << endl;
  debug << "sum_sio: " << sum_sio << endl;
  debug << "Z_s: " << Z_s << endl;
  debug << "w_s*Z_s: " << w_s*Z_s << endl;
  debug << "-------------" << endl;
  debug << "sum_p_diff: " << sum_s_diff << endl;
  debug << "sum_pio: " << sum_pio << endl;
  debug << "Z_p: " << Z_p << endl;
  debug << "w_p*Z_p: " << w_p*Z_p << endl;
  debug << "-------------" << endl;
  debug << "sum_fc_diff: " << sum_fc_diff << endl;
  debug << "sum_fc_ref: " << sum_fc_ref << endl;
  debug << "Z_fc: " << Z_fc << endl;
  debug << "w_fc*Z_fc: " << w_fc*Z_fc << endl;
  debug << "-------------" << endl;
  debug << "sum_fc3_diff: " << sum_fc3_diff << endl;
  debug << "sum_fc3_ref: " << sum_fc3_ref << endl;
  debug << "Z_fc3: " << Z_fc3 << endl;
  debug << "w_fc3*Z_fc3: " << w_fc3*Z_fc3 << endl;
  debug << "-------------" << endl;
